...
修改时间: 2026-02-11T21:56:35+08:00
---
## 无显示输出时重启 Linux
...
额外发现 [ifupdown-addons-interfaces(5) — ifupdown2 — Debian bookworm — Debian Manpages](https://manpages.debian.org/bookworm/ifupdown2/ifupdown-addons-interfaces.5.en.html) 有自带的拓展语句直接支持了，不再需要魔法般的 `post-up` 语句。
...
我 PVE 上的 vmbr0 实际上只挂了个 OpenWrt，所以禁用 snooping 其实也没什么广播群发劣化效应。但我还是保留*更佳*实践，采用 router。

设置这个条目直接 hardcode 不行，因为 PVE 启动时容器还没启动。那么就需要个 pct hookscript 来在容器启动时，设置这个。

- [How to use new hookscript feature? | Proxmox Support Forum](https://forum.proxmox.com/threads/how-to-use-new-hookscript-feature.53388/)
- [pve-docs/examples/guest-example-hookscript.pl at master · proxmox/pve-docs](https://github.com/proxmox/pve-docs/blob/master/examples/guest-example-hookscript.pl)

拎一个 AI 来 [写代码](https://github.com/enihsyou/Homelab/blob/main/pve/var/lib/vz/snippets/multicast-router.pl)，然后设置上就完了

```perl title="/var/lib/vz/snippets/multicast-router.pl"
#!/usr/bin/perl
