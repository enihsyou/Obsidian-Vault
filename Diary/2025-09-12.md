...
修改时间: 2025-09-12T03:39:10+08:00
---
...
也能用 MSYS2 的 ssh-add，见 [[#^daa4c1 |想要实现 Windows to MSYS2 也是可以的]]
...
想要实现 Windows to MSYS2 也是可以的，[[#Profile Script]] 通过在 MSYS2 的 /etc/profile 中运行 socat 后台的方式，不过需要用 MSYS2 中的 go 编译 npiperelay.exe 才行，否则会遇到 `copy from stdin to pipe failed: read /dev/stdin: The parameter is incorrect.` 的错误。 ^daa4c1
