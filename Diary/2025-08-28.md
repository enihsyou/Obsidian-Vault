...
- [IGMP Snooping (multicast forwarding) on AX routers? | SNBForums](https://www.snbforums.com/threads/igmp-snooping-multicast-forwarding-on-ax-routers.58245/)
- [asuswrt-merlin compatibility · troglobit/smcroute · Discussion #201](https://github.com/troglobit/smcroute/discussions/201) 这里是同样的报错，原因是不论路由器界面的 `LAN > IPTV > 启动组播路由` 开没开，这个进程都会启动

虽然 [Custom config files · RMerl/asuswrt-merlin.ng Wiki](https://github.com/RMerl/asuswrt-merlin.ng/wiki/Custom-config-files) 上没写支持 postconf，但 [代码上](https://github.com/RMerl/asuswrt-merlin.ng/commit/20dbf936f14c179d3063237b21cb7fb692f45102) 是可以的
