...
修改时间: 2026-02-15T02:16:17+08:00
---
...
应对依赖前一 K 线的缺陷，可以用跨周期引用来解决。不过此方法首先需要用通达信的“盘后数据下载”把数据补充完整，然后定义两个公式，分别写上：
...
```zig
IF(CLOSE > OPEN, DYNAINFO(3), TESTSKIP(0));
IF(CLOSE > OPEN, DYNAINFO(3), DRAWNULL);
```
