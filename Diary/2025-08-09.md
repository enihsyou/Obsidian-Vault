...
```
现在怀疑是 A(192.168.9.18) to B(192.168.2.82) 跨网段经路由器路由，B to A 直接链路直连不经过路由器导致的这个现象。无法关闭路由器的硬件加速，手上也没有可用的交换机，也就难以抓到LAN设备间的流量来验证。
通过在 192.168.9.18 上添加一条 192.168.2.82/32 的静态路由，也是能通的说明确实出在路由器上。
那么有两个解决思路
