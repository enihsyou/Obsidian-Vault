...
在开启了 Merlin Clash + Tproxy mode 的路由器上挂了一个与路由器同网段的 192.168.9.17 设备，上面运行了处在 192.168.2/24 网段的虚拟机，并且设备当做路由器处理转发。奇怪的是两个网段可以 ping 通但是无法建立 TCP 连接。
...
多绕个圈子，感觉非常蠢
2. 让 192.168.9.17 做 NAT，对外屏蔽子网信息，相当于同一子网间设备互访
当然能不 NAT 就不 NAT
