...
修改时间: 2026-03-21T20:05:17+08:00
---
## 迁移 Hyper-V 到 PVE
...
复制 p12 到 VM，并在 VM 中安装证书到本地计算机，再配置 RDP 使用证书：

```powershell
# Get the thumbprint of the SSL cert by its CN
$CN = "windows-vm.lan.kokomi.site"
$Thumbprint = (Get-ChildItem "Cert:\LocalMachine\My" | Where-Object {$_.Subject -match "CN=$CN" -and $_.NotAfter -gt (Get-Date)}).Thumbprint
...
如果第一步 Thumbprint 为空，检查是否导入到 `本地计算机\个人\证书`。如果导入证书时一路下一步，默认会在当前用户下，需要改成本地计算机；再检查下证书有没有过期。
...
```shellsession showLineNumbers collapse={2-51,71-122} "dwErrorStatus"
...
[Support create and distributing Root CRL · enihsyou/smallstep-certificates@4e04ec5](https://github.com/enihsyou/smallstep-certificates/commit/4e04ec5f942bdf8ecfc819367e3fb57b5cf9eed4)
[Support create and distributing Root CRL · enihsyou/smallstep-cli@4d259a7](https://github.com/enihsyou/smallstep-cli/commit/4d259a7e9f1a583c1c149fddf9566b2a99e71d55)

```shellsession "通过了分支证书吊销检查" showLineNumbers collapse={2-115}
...
```shell title="Deploy local compiled executable to PKI server"
scp -O step-ca pki:
ssh pki
chmod +x step-ca
setcap cap_net_bind_service=+ep step-ca
mv step-ca /usr/bin
rc-service step-ca restart
```
