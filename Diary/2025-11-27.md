...
修改时间: 2026-07-04T02:25:04+08:00
---
## 允许外部通过容器名解析至 Docker
...
`2026-07-04` 最好给容器设置上 `dns` 属性，为了避免潜在的 DNS 循环
...
```
       ...
       |                                                                 |
       |     +-------------------+              +-------------------+    |
       |     |      CoreDNS      |     (1)      |Docker Built-in DNS|    |
  +--> |     |  (192.168.9.53)   | -----------> |   (127.0.0.11)    |    |
  |    |     +-------------------+              +-------------------+    |
  |    |                                                  |              |
  |    +--------------------------------------------------|--------------+
  |                                                       |
  ...
  |                                                       v
  |            +----------------------------------------------------+
  |            |             Main Router (192.168.9.1)              |
  |            |                                                    |
  |    (3)     |               [ dnsmasq service ]                  |
  +------------|                                                    |
               |  Triggered Rule: server=/docker/192.168.9.53       |
               +----------------------------------------------------+
```

1. CoreDNS -> Docker Built-in DNS: CoreDNS 收到 `.docker` 请求，根据 `forward` 规则发送给 Docker 的内置 DNS (`127.0.0.11`)。
2. Docker Built-in DNS -> Main Router: `127.0.0.11` 找不到该容器，作为兜底机制，将请求向上转发给宿主机的上游 DNS（即主路由 `192.168.9.1`）。
3. Main Router -> CoreDNS: 主路由的 dnsmasq 接收到请求，命中 `server=/docker/192.168.9.53` 规则，将请求又强制转发回了 CoreDNS (`192.168.9.53`)。至此死循环闭合。
