...
选择通过组策略直接修改新标签页为 `chrome-search://local-ntp/local-ntp.html`，修改的位置在 `本子组策略编辑器 > 用户配置 > 管理模板 > Microsoft Edge > 启动、主页和新标签页 > 配置新的标签页 URL`
...
2. WAN6 接口，如果有大于/64的地址块，可以勾上扩展前缀，获取 IPv6-PD 地址块
3. LAN 接口 DHCP 服务器开启中继模式