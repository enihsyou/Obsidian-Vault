---
创建时间: 2026-01-29T23:41:48+08:00
修改时间: 2026-08-04T19:51:39+08:00
---
记录常用系统管理指令

## 镜像代理

### Debian

```shell
if [ -d /etc/apt/mirrors ]; then
    echo "https://mirrors.cernet.edu.cn/debian/" > /etc/apt/mirrors/debian/debian.list
    echo "https://mirrors.cernet.edu.cn/debian-security/" > /etc/apt/mirrors/debian/debian-security.list
else
    sed -i 's/deb.debian.org/mirrors.cernet.edu.cn/g' /etc/apt/sources.list /etc/apt/sources.list.d/debian.sources
    sed -i 's#security.debian.org#mirrors.cernet.edu.cn/debian-security#g' /etc/apt/sources.list /etc/apt/sources.list.d/debian.sources
fi
```

### Ubuntu

TDB

### Alpine

```shell
sed -i -E 's#(https?://)[^/]+/alpine#\1mirrors.huaweicloud.com/alpine#g' /etc/apk/repositories
```

### Node

```shell
npm config set registry https://registry.npmmirror.com/
pnpm config set registry https://registry.npmmirror.com/
```

### Rust

```shell
export RUSTUP_DIST_SERVER="https://rsproxy.cn"
export RUSTUP_UPDATE_ROOT="https://rsproxy.cn/rustup"
```

### PIP

```ini title="~/AppData/Roaming/pip/pip.ini"
[global]
cache-dir = G:/AppData/pip/cache
require-virtualenv = true
index-url = https://mirrors.aliyun.com/pypi/simple/
```

### UV

- [Settings | uv](https://docs.astral.sh/uv/reference/settings/#python-install-mirror)

```toml title="~/AppData/Roaming/uv/uv.toml"
python-downloads = "manual"
python-install-mirror = "https://mirror.nju.edu.cn/github-release/indygreg/python-build-standalone/"
cache-dir = "G:/AppData/uv/cache"

[[index]]
name = "aliyun"
url = "https://mirrors.cernet.edu.cn/pypi/web/simple"
default = true
exclude-newer = false
```

- `~/.dotfiles/language/uv/uv-windows.toml`

## vfox

Version Fox

```powershell
$env:VFOX_GOLANG_MIRROR = "https://mirrors.aliyun.com/golang/"
$env:VFOX_PYTHON_MIRROR = "https://mirrors.huaweicloud.com/python/"
$env:VFOX_NODEJS_MIRROR = "https://mirrors.aliyun.com/nodejs-release/"
```

## 清理缓存

### Rust

```shell
cargo install cargo-cache
cargo cache --autoclean-expensive
```

### Python

```shell
uv cache clean
pip cache purge
```

### Node

```shell
npm cache ls
npm cache clean --force

pnpm store path
pnpm store prune

# run inside folder with package.json
bun pm cache
bun pm cache rm
```

### DotNet

```shell
dotnet nuget locals all --list
dotnet nuget locals all --clear
```

## 证书管理

- [[2026-02-03#OpenWrt ACME 刷新证书提示 DNS 令牌失效]]
- [[2026-01-27#systemd certbot.service 状态异常]]
- [[2025-11-14#aria2 使用证书错误代码 80090327]]
- [[2025-08-08#给 Docker Daemon 启用 TLS 验证]]
- [[2025-08-30#强制 PVE 刷新 ACME 证书]]

```shell title="OpenWrt"
/etc/init.d/acme renew
```

```shell title="PVE"
systemctl start --no-block pve-daily-update.service
journalctl -u pve-daily-update.service -f
```

```shell title=Docker
# renew within validity
step ca renew ./cert.pem ./key.pem
# renew after certificate expired
step ca renew ./cert.pem ./key.pem --force
# renew with existing private key but different san
step certificate create "Docker Daemon" --san docker.lan.kokomi.site --san docker.pve.kokomi.site cert.csr --key key.pem --csr
step ca sign cert.csr cert.pem --not-after 8760h
```

```shell title=WSL2
sudo certbot renew
# aria2
sudo openssl pkcs12 -export -out /mnt/c/Users/enihsyou/.dotfiles/cli-app/aria2/aria2.pfx -in /etc/letsencrypt/live/amd-9700x-lan.lan.kokomi.site/cert.pem -inkey /etc/letsencrypt/live/amd-9700x-lan.lan.kokomi.site/privkey.pem -passout pass:
```

使用到 Cloudflare DNS API Token 的地方

- PVE, ACME
- docker，`/etc/private/certbot/cloudflare.ini`
- openwrt, `/etc/config/acme`, `/etc/acme/<domain>/<domain>.conf`
- wsl2, `~/.secrets/certbot/cloudflare.ini`
- ddns-go

> 注意设置了 [[2025-11-07#Limit Client IP Ranges for Cloudflare API Token]]

使用到 Cloudflare Account API Token 的地方
- RT-AX86U, Cloudflare DDNS Plugin

## 重启网络

```shell
systemctl status  systemd-networkd
systemctl restart systemd-networkd

# PVE
ifreload -a
```

## 关闭终端后继续运行

```shell
# 准备开始运行一个新任务
nohup python train_model.py > my_log.log 2>&1 &

# 任务已经跑起来了
$ long_running_script.sh
^Z
[1]+  Stopped                 long_running_script.sh
$ bg
[1]+ long_running_script.sh &
$ disown -h %1

# 在脚本内部启动另一个守护进程
setsid your_command > /tmp/log 2>&1 &
```