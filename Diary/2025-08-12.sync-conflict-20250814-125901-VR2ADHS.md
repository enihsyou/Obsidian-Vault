...
```shell showLineNumbers wrap=false collapse={3-18,32-999}
...
```nginx title=/etc/config/firewall showLineNumbers wrap=false collapse={3-18,32-999}
config defaults
	option syn_flood '1'
	option input 'ACCEPT'
...
```nginx title=/etc/config/rg_firewall
