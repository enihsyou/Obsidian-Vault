...
这里从设备上发了个内网请求，在路由器上抓包，在路由器上一见到链接就是 ESTABLISHED 状态，而不是 SYN_SENT ，所以进入了 INVALID state。
...
怀疑是 A(192.168.9.18) to B(192.168.2.82) 跨网段，A 没有相关路由表，就交给默认路由器路由；B to A 链路未经过路由器导致的这个现象。
