...
修改时间: 2026-08-07T01:37:21+08:00
---
...
`rjd` 不只是自动 DHCP，主要还负责：

- AutoWAN：探测活动端口、DHCP/PPPoE、默认网关及上联口切换。
- 监听 `network.interface`、`network.restart`、`network.workmode` 等事件。
- 网络变化时执行 LAN/端口复位、PHY 断电重上电。
- 协调无线重启及部分 ACL/客户端恢复。
...
所以 `114.114.114.114` 不是上级 DHCP 下发的，而是 netifd 在只有一个 IPv4 DNS 时主动补上的。修改 dnsmasq 的 `noresolv` 不能阻止 netifd 把它写进文件。
...
## pwn.college 环境自动加载 .bashrc

#pwn_college

之前写了 [[Diary/2026-03-24#pwn.college 在 SSH 获取 Desktop 的剪贴板内容]] 等内容，放在 `~/.bashrc`，然后发现光 `ssh host` 过去并不会加载此文件，还得手动启动个 bash 才行。

其实只需要在远程机器的 `~/.bash_profile` 添加：
...
原理是这样，也可以是 `/etc/profile`、`~/.bash_profile`、`~/.bash_login`、`~/.profile`
...
但这还不够，如果 `ssh host env` 运行命令还是得不到在 .bashrc 中设置的环境变量。

虽然我的目标是加载 `PYTHONPATH`，这个可以利用 `python -m site` 第一个输出 pwd 的特性，把要加载的包放在当前目录。

想要完美还得让 ssh 自己加载环境变量，在环境里入口不是 SSH 进来的，而是被放入 [`run/dojo/bin/ssh-entrypoint`](https://github.com/pwncollege/dojo/blob/625883b7e2ff011a93d1335b575527672d7d15a4/sshd/enter.py#L110) 启动的容器里

```shell title=/run/dojo/bin/ssh-entrypoint
#!/nix/store/f43k3lffqlz3n864inxz8zf28jvks1q6-bash-interactive-5.3p3/bin/bash
...
以 `docker exec -it --workdir=/home/hacker /bin/bash -c command` 的形式运行

所以结论是得主动设置 `ssh host ENV=xxx command`